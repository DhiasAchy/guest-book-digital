<?php
    ...
    class Event extends CI_Controller
    {
    	...
    	public function check_in() {
			...
			// echo $guest_qr." | ".$event_id;
    	    
    	    // search guest
    	    // $guest_data = $this->Model_event_guest->getall_by_parameter($guest_qr);
    	    $guest_data = $this->Model_event_guest->getguest_checkin_data($guest_qr,$event_id);
			$count = count($guest_data);
			// print_r($guest_data);
			...
			if ($count>0) {
				foreach ($guest_data as $key => $value) {
					...
					// echo $value['event_update']."<br>";
					if ($value['event_update']==null) {
						$data = array(
							'event_id'       => $event_id,
							'guest_id'       => $value['id'],
							'guest_name'     => $value['name'],
							'guest_address'  => $value['address'],
							'guest_time'     => $value['event_update']
						);
						...
						$this->db->insert('app_guest_log', $data_log);
						// $this->db->update('app_guest_event', ['updated_at' => date('Y-m-d H:i:s')], ['id' => $value['id'], 'event_id' => $event_id]);
						
						// update event attendace
						$this->db->set('updated_at', date('Y-m-d H:i:s'));
						$this->db->where('guest_id', $value['id']);
						$this->db->where('event_id', $event_id);
						$this->db->update('app_guest_event');

						// return
						$json = ['error' => false, 'msg' => 'Tamu sesuai dan terdaftar.'];
					} else { $json = ['error' => true, 'msg' => 'Tamu sudah pernah scan QR.']; }
				}
			} else { $json = ['error' => true, 'msg' => 'Tamu tidak terdaftar.']; }
