...
class Event extends CI_Controller
{
    ...
    public function detail($id = null,$json = null) 
    {
        if (!empty($id)) {
            ...
            // $data['guest'] = $this->db->get_where('app_guest_event',['event_id' => $id])->row_array();
            $data['guest_'] = $this->Model_event_guest->getallguest_byevent($id);
    ...
    public function add_guest($id=null)
    {
        ...
        $this->template->admin('admin/event/event/add_guest',$data);
    }
    ...
    public function insert() 
    {
        ...
        $this->form_validation
            ...
            ->set_rules('code', 'Kode Event', 'trim|required')
    ...
    public function insert_guest()
    {
        ...
        $post = $this->input->post(null, false);
        $this->form_validation
            ->set_rules('states[]', 'Nama Peserta', 'trim|required')
            ->set_error_delimiters('', '');

        if ($this->form_validation->run() == FALSE) {
            $json = ['error' => true, 'msg' => $this->form_validation->get_errors(), 'form_validation' => true];
        } else { 
            $array_guest = $post['states'];
            // echo $array_guest;
            foreach ($array_guest as $key => $value) {
                // echo $value."<br>";

                // cek di db sudah ada belum
                ...
                // echo $query->num_rows();
                if ($query->num_rows()>0) {
                    $json = ['error' => true, 'msg' => 'Data sudah ada.'];
                } else {
                    $data = array(
                        'guest_id'   => $value,
                    ...
                    $this->db->insert('app_guest_event', $data);
                    $json = ['error' => false, 'msg' => 'Data berhasil disimpan.'];
                }
            }
        }
        $this->output->set_content_type('application/json')->set_output(json_encode($json));
    ...
    public function send_email() 
    {
        ...
        // $this->email->to('dhias.wirawangroup@gmail.com');
       ...
       $mail->SMTPAuth = true;
       $mail->Username = 'info@wirawan.web.id'; // user email
       $mail->Password = '7^FxlNSn'; // password email
       $mail->SMTPSecure = 's   sl';
       $mail->Port     = 465;
       ...
       // Add a recipient
       $mail->addAddress('mohamadrizkyadhiasta@gmail.com'); //email tujuan pengiriman email
